...
describe("groups", () => {
  ...
  describe("should validate only properties of the given group: title-validation", () => {
    it("with valid title", () => {
      return validator.validate(validTitle, { groups: ["title-validation"] })
        .then((errors) => {
          expect(errors.length).toEqual(0);
        });
    });
    ...
    it("with valid text", () => {
      return validator.validate(validText, { groups: ["title-validation"] })
        .then((errors) => {
          expect(errors.length).toEqual(1);
    ...
    it("with both valid", () => {
      return validator.validate(validBoth, { groups: ["title-validation"] })
        .then((errors) => {
          expect(errors.length).toEqual(0);
        });
    });
    ...
    it("with none valid", () => {
      return validator.validate(validNone, { groups: ["title-validation"] })
        .then((errors) => {
          expect(errors.length).toEqual(1);
          expectTitleContains(errors[0]);
        });
  ...
  describe("should validate only properties of the given group: text-validation", () => {
    it("with valid title", () => {
      return validator.validate(validTitle, { groups: ["text-validation"] })
        .then((errors) => {
          expect(errors.length).toEqual(1);
    ...
    it("with valid text", () => {
      return validator.validate(validText, { groups: ["text-validation"] })
        .then((errors) => {
          expect(errors.length).toEqual(0);
        });
    });
    ...
    it("with both valid", () => {
      return validator.validate(validBoth, { groups: ["text-validation"] })
        .then((errors) => {
          expect(errors.length).toEqual(0);
        });
    });
    ...
    it("with none valid", () => {
      return validator.validate(validNone, { groups: ["text-validation"] })
        .then((errors) => {
          expect(errors.length).toEqual(1);
          expectTextContains(errors[0]);
        });
  ...
  describe("should validate only properties of the given groups: both groups", () => {
    it("with valid title", () => {
      return validator.validate(validTitle, {
    ...
    it("with valid text", () => {
      return validator.validate(validText, {
        groups: ["title-validation", "text-validation"],
      }).then((errors) => {
        expect(errors.length).toEqual(1);
        expectTitleContains(errors[0]);
    ...
    it("with both valid", () => {
      return validator.validate(validBoth, {
        groups: ["title-validation", "text-validation"],
      }).then((errors) => {
        expect(errors.length).toEqual(0);
      });
    ...
    it("with none valid", () => {
      return validator.validate(validNone, {
        groups: ["title-validation", "text-validation"],
      }).then((errors) => {
  ...
  describe("multiple groups per property", () => {
    ...
    describe("group: contains", () => {
      it("with valid contains", () => {
        return validator.validate(validContains, { groups: ["contains"] }).then(
          (errors) => {
            expect(errors.length).toEqual(0);
          },
        );
      ...
      it("with valid matches", () => {
        return validator.validate(validMatches, { groups: ["contains"] }).then(
          (errors) => {
            expect(errors.length).toEqual(1);
            expectTitleContains(errors[0]);
          },
      ...
      it("with valid both", () => {
        return validator.validate(validBoth, { groups: ["contains"] }).then(
          (errors) => {
            expect(errors.length).toEqual(0);
          },
        );
      ...
      it("with valid none", () => {
        return validator.validate(validNone, { groups: ["contains"] }).then(
          (errors) => {
            expect(errors.length).toEqual(1);
            expectTitleContains(errors[0]);
          },
    ...
    describe("group: matches", () => {
      it("with valid contains", () => {
        return validator.validate(validContains, { groups: ["matches"] }).then(
          (errors) => {
            expect(errors.length).toEqual(1);
      ...
      it("with valid matches", () => {
        return validator.validate(validMatches, { groups: ["matches"] }).then(
          (errors) => {
            expect(errors.length).toEqual(0);
          },
        );
      ...
      it("with valid both", () => {
        return validator.validate(validBoth, { groups: ["matches"] }).then(
          (errors) => {
            expect(errors.length).toEqual(0);
          },
        );
      ...
      it("with valid none", () => {
        return validator.validate(validNone, { groups: ["matches"] }).then(
          (errors) => {
            expect(errors.length).toEqual(1);
            expectTitleMatches(errors[0]);
          },
    ...
    describe("groups: contains & matches", () => {
      it("with valid contains", () => {
        return validator.validate(validContains, {
          groups: ["contains", "matches"],
        }).then((errors) => {
          expect(errors.length).toEqual(1);
          expectTitleMatches(errors[0]);
        });
      ...
      it("with valid matches", () => {
        return validator.validate(validMatches, {
          groups: ["contains", "matches"],
        }).then((errors) => {
          expect(errors.length).toEqual(1);
          expectTitleContains(errors[0]);
  ...
  describe("ValidationOptions.always", function () {
    ...
    it("should validate decorator without options", function () {
      return validator.validate(model1, { always: true, groups: ["A"] }).then(
        (errors) => {
    ...
    it("should not validate decorator with groups if validating without matching groups", function () {
      return validator.validate(model2, { always: true, groups: ["B"] }).then(
        (errors) => {
    ...
    it("should not validate decorator with always set to false", function () {
      return validator.validate(model3, { always: true, groups: ["A"] }).then(
        (errors) => {
          expect(errors).toHaveLength(0);
        },
      );
    ...
    it("should validate decorator with always set to true", function () {
      return validator.validate(model4, { always: true, groups: ["A"] }).then(
        (errors) => {
          expect(errors).toHaveLength(1);
        },
      );
  ...
  describe("strictGroups", function () {
    ...
    it("should ignore decorators with groups if validating without groups", function () {
      return validator.validate(model1, { strictGroups: true }).then(
        (errors) => {
          expect(errors).toHaveLength(0);
        },
      );
    ...
    it("should ignore decorators with groups if validating with empty groups array", function () {
      return validator.validate(model1, { strictGroups: true, groups: [] })
        .then((errors) => {
          expect(errors).toHaveLength(0);
        });
    });
    ...
    it("should include decorators with groups if validating with matching groups", function () {
      return validator.validate(model1, { strictGroups: true, groups: ["A"] })
        .then((errors) => {
          expect(errors).toHaveLength(1);
          expectTitleContains(errors[0]);
        });
    ...
    it("should not include decorators with groups if validating with different groups", function () {
      return validator.validate(model1, { strictGroups: true, groups: ["B"] })
        .then((errors) => {
          expect(errors).toHaveLength(0);
        });
    });
  ...
  describe("always", () => {
    ...
    it("should always validate a marked field even if another group is specified", () => {
      return validator.validate(model, { groups: ["sometimes"] }).then(
        (errors) => {
          expect(errors.length).toEqual(2);
          expectTitleContains(errors[0]);
          expectTextContains(errors[1]);
    ...
    it("should always validate a marked field if its group is specified also (doubly enabled)", () => {
      return validator.validate(model, { groups: ["always"] }).then(
        (errors) => {
          expect(errors.length).toEqual(1);
          expectTextContains(errors[0]);
        },
  ...
  describe("groups - nested", () => {
    ...
    function expectChildConstraint(
      ...
      expect(error.children[0].constraints).toEqual({
...
describe("context", () => {
  ...
  it("should stop at first error.", () => {
    ...
    const model = new MyClass();
    return validator.validate(model, { stopAtFirstError: true }).then(
      (errors) => {
        console.log();
        expect(errors.length).toEqual(1);
